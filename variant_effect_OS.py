...
def main(
    ...
    ism: Optional[bool] = True,
    scan: Optional[bool] = False,
):
    results_dir = "/s/chromatin/a/nobackup/ahmed/DeepPlant/results/ISM_OS"
    ...
    if ism:
        ...
        config = ExpressionConfig(
            **read_json(
                json_path="/s/chromatin/m/nobackup/ahmed/DeepPlant/json/config_OS_expressionRC.json"
            )
        )
        eps = 1e-4
        model = build_model(
            args=config,
            new_model=False,
            model_path="/s/chromatin/a/nobackup/ahmed/DeepPlant/results/expression_OS/148391/model_25_10_30:13:02.pt",
        ).to(device)
        if n_gpu > 1:
            setup(device, n_gpu)
            model = DDP(
...
class load_dataset:
    def __init__(
        ...
        expression_df = pd.read_csv(
            "/s/chromatin/m/nobackup/ahmed/DeepPlant/data/oryza/expression/expression_data_TSS_centered_2500_0.csv"
        )

        expression_df["gene"] = list(map(get_gene, expression_df.header))
        expression_df["tss"] = list(map(get_tss, expression_df.header))
        df = pd.read_csv(
            "/s/chromatin/m/nobackup/ahmed/DeepPlant/data/oryza/oryza_expression_data_split.csv"
        )
        self.test_genes = df.loc[df.split == "test"].gene.values
        print(self.test_genes[0])
